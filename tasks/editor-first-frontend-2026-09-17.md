...
| 5 | **审批卡自研，排在最前** | 官方审批只由被我们禁用的 `ui-chat` 提供，可能是回归。**⚠️ 2026-09-17 该前提被 I1.1 证伪：官方另有 `dsh-client-ui-approval`，已加载且未被禁用 —— 见 I1.1 阻塞块，本行待用户裁定后改写。** |
...
- [ ] **I1.1 RED：确认审批回归** — 目标：用证据证明「现在没有审批渲染者」，而不是靠推断。
  ...
  > - **要用户选一个**（详见证据 §5）：① **撤销 I1.2/I1.3，改为「验证并修好复用路径」**（推荐）；
  >   ② 保留 I1.2 但降级为「只改外观与作者语言，复用官方应答与数据通路」；③ 照原计划整张自研
  >   （目前没有任何证据支持「官方那张不够」）；④ 先只补「一次真实审批」的实验，拿结果再定。
  >
  > **若选 ③，会与 §2「不建第二套 …审批…」直接冲突**，需要用户明确豁免。
  > **I1.1 未完成**（真实审批那一次实验没跑）。Phase 1 其余增量与 I0.2 之后的排期取决于上面的选择。loop 已按 §5 停止。
