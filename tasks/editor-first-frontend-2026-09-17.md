...
- ~~**I2.3 `/` 与 `@` 候选菜单**~~ → **已重切为下面的 I2.4 + I2.5（换序）**。原条目保留为死锁的查证记录。  - 文件：`NovelComposer.tsx`、`novel-workbench-composer.spec.ts`（扩展）
  ...
  >
  > **✅ 用户裁定（2026-09-17）：走路线 2 —— 我们持有草稿，只做"应用选中"这一层。**
  > 候选仍来自官方管道（`ctx.inputTriggers` 的 `track` + `menu` 是**可读**的，这半没被挡）；选中后由我们
  > 自己把文本插进底栏的 textarea。计划里「不重建输入状态机」据此放宽为「**只**重建应用层」——
  > 理由不是排序，而是**官方输入 API 是闭的**。
  >
  > **顺带查清一个本会撞上的冲突，并已解决（不用再问）：** I2.4「停渲染官方会话面」会让
  > **I1 复用的官方审批面板失去座位** —— 它的注册目标是 `conversation.composer`。
  > 但官方 DOM 里这两件事是**分开的**：
  > - 官方 **InputBar 渲染进 `conversation.composer.bar`**（由官方会话面自己 `renderSlot`）；
  > - **审批面板注入的是父槽 `conversation.composer`**。
  >
  > 所以解法是：**我们的 frame 自己声明并渲染 `conversation.composer`**（审批面板留住，I1 的决定不破），
  > **不声明也不渲染 `conversation.composer.bar`**（官方输入框根本不出现 → 没有重复输入框）。
  > I2.4 按这个写法推进，不需要再让用户选一次。
