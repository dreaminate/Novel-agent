...
- [ ] **I1.1 RED：确认审批回归** — 目标：用证据证明「现在没有审批渲染者」，而不是靠推断。
  ...
  >
  > **⚠️ 下一步要花模型额度。** 真实审批需要一次带工具调用的**真实模型会话**，
  > 是本计划里第一个真正产生模型调用费用的动作。**循环在此停下等用户授权。**
  > 可选的免费替代口径（若用户愿意）：先用插件安装/结算那条 **host 侧** `requiresApproval` 路径试一次。
  >
  > **I1.1 仍未完成**（真实审批没跑）。**本轮同样没有花模型额度。**
